    ...
    if (gl_state) {
        ...
        if (rtarget == GL_PROXY_TEXTURE_2D) {
            ...
            case GL_TEXTURE_WIDTH:
                // gl_state->proxy_width was already stored as the dimension
                // FOR THIS LEVEL by the matching glTexImage2D(PROXY, level, ...,
                // width, height, ...) call (see the PROXY_TEXTURE_2D branch
                // above). Shifting it right by `level` again here mip-reduces
                // it a second time, so a caller probing level L back-to-back
                // (set then get, the only pattern the driver's PROXY target is
                // meant to support -- there is no per-level storage, just this
                // one shared "last call" slot) reads a width shrunk by 2^L
                // instead of 2^0. Minecraft's stitcher uses exactly this
                // set/get pair to find the highest mip level the GPU can hold
                // for the atlas: the quadratic error stays invisible at level
                // 0 (shifting by 0 is a no-op) but grows with every extra
                // Mipmap level, so raising Mipmap makes the probe's answer
                // flip between runs and the atlas keeps getting rebuilt at a
                // different resolution -- the flicker. Return the stored
                // value as-is.
                (*params) = (float)gl_state->proxy_width;
                return;
            case GL_TEXTURE_HEIGHT:
                (*params) = (float)gl_state->proxy_height;
                return;
...
void glGetTexLevelParameteriv(GLenum target, GLint level, GLenum pname, GLint* params) {
    ...
    if (gl_state) {
        ...
        if (rtarget == GL_PROXY_TEXTURE_2D) {
            switch (pname) {
            case GL_TEXTURE_WIDTH:
                // See the identical case in glGetTexLevelParameterfv above:
                // proxy_width already IS the level-`level` dimension, set by
                // the glTexImage2D(PROXY_TEXTURE_2D, level, ...) call this get
                // is meant to immediately follow. Re-applying nlevel() here
                // shifted it down by `level` a second time, so the reported
                // width shrank as size >> (2 * level) instead of size >>
                // level. That is invisible at level 0 but compounds with
                // every higher mip level probed, which is exactly what
                // Minecraft's atlas stitcher does when "Mipmap Levels" is
                // raised -- it re-derives a different (wrong) answer for the
                // highest usable level each time it probes, so the atlas gets
                // rebuilt at a different resolution over and over: the
                // flicker. Return the stored per-level value unshifted.
                (*params) = gl_state->proxy_width;
                return;
            case GL_TEXTURE_HEIGHT:
                (*params) = gl_state->proxy_height;
                return;
            case GL_TEXTURE_INTERNAL_FORMAT:
